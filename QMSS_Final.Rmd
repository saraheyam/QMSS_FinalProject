...
preds <- c("last_name", "first_name", "Gender", "Graduation.year", 
           "Prim.Schl", "Credential", "Num.Secondaries", "Quality_Score_Org", 
           "ACI_Score_Org", "IA_Score_Org", "MIPS_Score_Org", 
           "Quality_Score_Dr", "ACI_Score_Dr", "IA_Score_Dr", 
           "MIPS_Score_Dr", "avg.perf", "MIPS75_Dr")
...
#training <- training[, c(3:4, 6, 8, 12:15, 17:22)]
#testing <- testing[, c(3:4, 6, 8, 12:15, 17:22)]
```

```{r}
interaction(no_tasks$Primary.specialty, no_tasks$Credential, no_tasks$Medical.school.name, sep = ":")
```
...
train_fctr <- train_fctr[, c(3:8, 12:15, 17:22)]
test_fctr <- test_fctr[, c(3:8, 12:15, 17:22)]
```
...
pp_names <- c("Num.Secondaries", "Quality_Score_Org", "IA_Score_Org", 
              "Quality_Score_Dr", "IA_Score_Dr", "avg.perf")
...
ols_strat <- lm(MIPS75_Dr == "yes" ~ ., data = train_fctr)
yhat_strat <- predict(ols_strat, newdata = test_fctr)
z_ols_strat <- factor(yhat_strat > 0.5, levels = c(TRUE, FALSE), 
                labels = c("yes", "no"))
confusionMatrix(z_ols_strat, reference = test_fctr$MIPS75_Dr)
```
...
calibration(MIPS75_Dr ~ yhat, data = testing) %>%
  plot(., main = "Without School or Primary Specialty Name")

calibration(MIPS75_Dr ~ yhat_strat, data = test_fctr) %>%
  plot(., main = "With Stratification by Factor")
```
...
```{r}
# Cannot use non-cross-validated 
# must remove creds/school name/primary speciality 
# [4, 5, 7]
...
training <- training[, c(3, 6,  8, 12:15, 17:22)]
testing <- testing[, c(3, 6, 8, 12:15, 17:22)]
...
train_fctr <- train_fctr[, c(3:8, 12:15, 17:22)] %>% distinct(.)
test_fctr <- test_fctr[, c(3:8, 12:15, 17:22)] %>% distinct(.)
```


```{r, message = F, warning = F, cache = T}
logit_strata <- glm(MIPS75_Dr ~ ., data = train_fctr, family = binomial(link = "logit"))
z_strata <- predict(logit_strata, newdata = test_fctr, type = "response") > 0.5
z_strata <- factor(z_strata, levels = c(TRUE, FALSE), labels = c("no", "yes"), order = T)
confusionMatrix(z_strata, test_fctr$MIPS75_Dr)

# too many factors just not in both training and testing, level-wise

logit <- glm(MIPS75_Dr ~ ., data = training, family = binomial(link = "logit"))
z <- predict(logit, newdata = testing, type = "response") > 0.5
z <- factor(z, levels = c(TRUE, FALSE), labels = c("no", "yes"), order = T)
...
enet_strata <- train(formula(logit_strata), data = train_fctr, method = "glmnet",
              trControl = ctrl, tuneLength = 10, preProcess = pp)
enet_hat_strata <- predict(enet_strata, newdata = test_fctr)
confusionMatrix(enet_hat_strata, reference = test_fctr$MIPS75_Dr)
...
```
```{r}
poly_cut <- c("Num.Secondaries", "ACI_Score_Dr", "AID_Score_Org", "MIPS_Score_Dr", "MIPS_Score_Org")
...
# with polynomials and cuts

poly_strat <- glm(MIPS75_Dr ~ ., data = train_fctr, family = binomial(link = "logit"))
poly_z_strat <- predict(poly_strat, newdata = test_fctr, type = "response") > 0.5
poly_z_strat <- factor(poly_z_strat, levels = c(TRUE, FALSE), labels = c("no", "yes"), order = T)
confusionMatrix(poly_z_strata, test_fctr$MIPS75_Dr)

# too many factors just not in both training and testing, level-wise

poly <- glm(MIPS75_Dr ~ ., data = training, family = binomial(link = "logit"))
poly_z <- predict(poly, newdata = testing, type = "response") > 0.5
poly_z <- factor(poly_z, levels = c(TRUE, FALSE), labels = c("no", "yes"), order = T)
confusionMatrix(poly_z, testing$MIPS75_Dr)
```

```{r, message = F, warning = F, cache = T}
# glmnet PENALIZATION
# with polynomials and uts

ctrl <- trainControl(method = "cv", number = 3)

poly_el_strat <- train(formula(poly_strata), data = train_fctr, method = "glmnet",
              trControl = ctrl, tuneLength = 10, preProcess = pp)
poly_yh_strat <- predict(poly_el_strat, newdata = test_fctr)
confusionMatrix(poly_yh_strat, reference = test_fctr$MIPS75_Dr)


poly_el <- train(formula(poly), data = training, method = "glmnet",
              trControl = ctrl, tuneLength = 10, preProcess = pp)
